...
void __cilkrts_init_full_frame(full_frame *ff, __cilkrts_stack_frame *sf)
{
  ...
  ff->frame_type = FF_DEFAULT_TYPE;
  // Do we really need to initialize this root field?  Probably not,
  // since we initialize it lazily, but I'm not sure we save much by
  // not doing so...
  ff->pipe_iter_root = NULL;
  ff->full_frame_magic_1 = FULL_FRAME_MAGIC_1;

  __cilkrts_reset_full_frame(ff, sf);
}
                                      ...
                                      __cilkrts_stack_frame *sf)
{
  ...
  START_INTERVAL(w, INTERVAL_ALLOC_FULL_FRAME) {
    ...
    __cilkrts_init_full_frame(ff, sf);
...
void __cilkrts_destroy_full_frame(__cilkrts_worker *w, full_frame *ff)
{
  ...
  CILK_ASSERT(FF_PIPE_ITER != ff->frame_type);
