...
global_state_t* cilkg_get_user_settable_values()
{
    ...
    if (! cilkg_user_settable_values_initialized)
    {
        ...
        // Set default options for pinning of worker threads.
        g->pin_options.policy = PIN_NONE;    // Don't pin workers.
        g->pin_options.verbose = 0;          // No output.
...
#endif
        // Undocumented options: check whether we want to pin threads.
        pinning_parse_options(&g->pin_options);
