...
static inline int full_frame_is_suspended(full_frame *ff)
{
    return (ff->call_stack != NULL);
}

static inline int full_frame_is_suspended_for_pipe(full_frame *ff)
{
    return (full_frame_is_suspended(ff) &&
            ((FF_PIPE_SUSPENDED_CONTROL == ff->resume_flags) ||
             (FF_PIPE_SUSPENDED_ITER == ff->resume_flags)));
}
...
/* Returns true if the implicit steal flags for this worker are
 * clear. */
static int worker_implicit_steal_flags_cleared(__cilkrts_worker *w)
{
    return !(w->l->implicit_deque_flags.parent_resume ||
             w->l->implicit_deque_flags.iter_ff_on_top);
}
...
static int ff_is_pipe_iter_descendant(full_frame *ff)
{
    return ((FF_PIPE_ITER == ff->frame_type) ||
            (FF_PIPE_ITER_CALL_DESCENDANT == ff->frame_type));
}
...
static void make_runnable(__cilkrts_worker *w, full_frame *ff, deque* d)
{
    d->frame_ff = ff;
    w->l->implicit_deque_flags.iter_ff_on_top =
            (char)ff_is_pipe_iter_descendant(ff);
...
/*
 * Same as __cilkrts_push_next_frame, except it doesn't increment the
 * join counter.
 */
static void __cilkrts_push_next_suspended_frame(__cilkrts_worker *w, full_frame *ff)
{
    CILK_ASSERT(ff);
    CILK_ASSERT(!w->l->next_frame_ff);
    w->l->next_frame_ff = ff;
...
static void link_child_ff(__cilkrts_worker *w,
                              ...
                              cilk_fiber *fiber) 
{
    ...
    link_child_ff(w, parent_ff, fiber, child_ff);
    return child_ff;
}

// Similar to make child, except we are creating a child
// full frame for a pipeline iteration.
// Note that the stack frame argument to make_pipe_iter_child
// differs: it takes in a stack frame for the parent, not
// the child.
static full_frame *make_pipe_iter_child(__cilkrts_worker *w,
                                        full_frame *control_ff,
                                        __cilkrts_stack_frame *control_sf,
                                        cilk_fiber *fiber)
{
    full_frame *child_ff =
            piper_create_iter_ff_for_stolen_control_frame(w,
                                                          control_sf,
                                                          control_ff);
    push_child(control_ff, child_ff);
    link_child_ff(w, control_ff, fiber, child_ff);
    return child_ff;
}
                                     ...
                                     __cilkrts_stack_frame *const loot_sf)
{
    ...
    // If the oldest full frame is a pipeline iteration, or connected
    // to one by a call chain, then make one last pass to set the
    // pipe_iter_root field.
    if (ff_is_pipe_iter_descendant(oldest_ff)) {
        full_frame* current_ff = ff;
        while (current_ff != oldest_ff) {
            PIPER_DBG_PRINTF(1,
                             "W=%d: frame = %p. marked pipe descendant, root will be %p\n",
                             w->self, current_ff, oldest_ff);
            CILK_ASSERT(current_ff->is_call_child);
            current_ff->frame_type = FF_PIPE_ITER_CALL_DESCENDANT;
            current_ff->pipe_iter_root = oldest_ff->pipe_iter_root;
            current_ff = current_ff->parent;
        }
    }
                             ...
                             deque* d, cilk_fiber* fiber)
{
    ...
    BEGIN_WITH_FRAME_LOCK(w, parent_ff) {
        ...
        // If the stolen frame is marked as a Piper frame, then we
        // just stole the control frame of a pipe_while loop.
        //
        // The only exception is if we are detaching ourselves.
        // If w==victim, we are running under CILK_FORCE_REDUCE=1,
        // and this path assumes we are executing serially.
        // Then, a pipeline loop should behave like an ordinary
        // serial loop that spawns each iteration.  None of the
        // special pipeline paths in the runtime should
        // execute.
        int simulated_steal = (w == victim);
        int stealing_pipe_control = (sf->flags & CILK_FRAME_PIPER);
        int create_special_pipe_iter_frame = stealing_pipe_control && (!simulated_steal);

        if (create_special_pipe_iter_frame) {
          // When the loot stack frame is the control frame of
          // a pipe_while loop, the full frame for the iteration
          // is created from the control buffer.

          PIPER_DBG_PRINTF(1,
                           "W=%d, victim=%d, just stole a control frame sf=%p, sf->flags=%p, loof_sf = %p, loot_call_stack=%p, parent_ff=%p\n",
                           w->self, victim->self,
                           sf,
                           (void*)(size_t)sf->flags,
                           loot_ff,
                           loot_ff->call_stack,
                           parent_ff);

          child_ff = make_pipe_iter_child(w,
                                          loot_ff,
                                          sf,
                                          fiber);
        }
        else {
          // Normal execution --- create a child full frame off the
          // heap.
          child_ff = make_child(w, loot_ff, 0, fiber);
          CILK_ASSERT(child_ff->frame_type == FF_DEFAULT_TYPE);

          // This flag gets set here if a simulated steal
          // happens and child_ff is a child of a control frame.
          // This bit is only interesting on the force reduce path.
          // We want to mark the frame as a pipe iter frame, but not
          // go through all the special execution paths.
          if (stealing_pipe_control && simulated_steal) {
            child_ff->frame_type = FF_PIPE_ITER_SIMULATED;
          }
        }

        PIPER_DBG_PRINTF(1,
                         "W=%d, victim=%d, steal: parent_ff=%p, loot_ff=%p, child_ff=%p\n",
                         w->self, victim->self, parent_ff, loot_ff, child_ff);
        ...
        BEGIN_WITH_FRAME_LOCK(w, child_ff) {
            ...
            if (!create_special_pipe_iter_frame) {
              // Installation of a child_ff for a normal steal.
              ...
              // This function behaves like the make_runnable call
              // above, except it performs some extra
              // synchronization and setting of worker flags
              // because we are installing a pipeline iteration.
              piper_install_iteration_frame(w,
                                            sf,
                                            child_ff,
                                            victim);
            }
...
/**
 * Gets a new fiber to resume a full frame, if that frame doesn't have
 * one.
 *
 * @return 1 if we needed a new fiber.
 *         0 otherwise.
 */
static int possibly_fill_fiber_for_next_ff(__cilkrts_worker *w,
                                           full_frame* next_ff)
{
    if (!next_ff->fiber_self) {
        cilk_fiber* fiber = cilk_fiber_allocate(&w->l->fiber_pool);
        CILK_ASSERT(fiber);
        cilk_fiber_reset_state(fiber,
                               fiber_proc_to_resume_user_code_for_random_steal);

        PIPER_DBG_PRINTF(2,
                         "W=%d: next_ff=%p, just allocated fiber %p\n",
                         w->self, next_ff, fiber);

        next_ff->fiber_self = fiber;
        return 1;
    }
    return 0;
}

// Returns 1 if we successfully stole something, AND
// we need the fiber.
static int pipeline_check_right_steal(__cilkrts_worker *w,
                                      __cilkrts_worker *victim,
                                      full_frame *iter_ff_desc,
                                      cilk_fiber *fiber,
                                      int* using_fiber)
{


    full_frame* iter_ff = iter_ff_desc->pipe_iter_root;
    CILK_ASSERT(iter_ff);

    int success = 0;
    full_frame* next_ff = piper_try_implicit_steals(w,
                                                    victim,
                                                    iter_ff);

    PIPER_DBG_PRINTF(3,
                     "W=%d, victim=%d: pipe steal got next_ff = %p. with call stack %p\n",
                     w->self, victim->self, next_ff,
                     next_ff ? next_ff->call_stack : NULL);

    if (next_ff) {
        success = 1;
        CILK_ASSERT(!victim->l->implicit_deque_flags.parent_resume);

        // Inherit the team of the victim.
        // Implicit steals can only happen between
        w->l->team = victim->l->team;
        w->l->cached_team = w->l->team;
        CILK_ASSERT(NULL != w->l->cached_team);
        __cilkrts_push_next_suspended_frame(w, next_ff);

        // Save in the fiber if we don't have one.
        if (!next_ff->fiber_self) {
            next_ff->fiber_self = fiber;
            *using_fiber = 1;
        }
...
static void random_steal(__cilkrts_worker *w, steal_t s)
{
    ...
    if (!can_steal_from(victim, d) &&
            worker_implicit_steal_flags_cleared(victim)) {
    ...
    if (worker_trylock_other(w, victim)) {
        ...
        {
            // First try pipeline steal --- checking implicit flags.
            if (s == STEAL_CORE) {
                if (!worker_implicit_steal_flags_cleared(victim)) {
                    full_frame *iter_ff_desc = victim->l->core_frame_ff;
                    if (iter_ff_desc) {
                        CILK_ASSERT(ff_is_pipe_iter_descendant(iter_ff_desc));
                        success = pipeline_check_right_steal(w,
                                                             victim,
                                                             iter_ff_desc,
                                                             fiber,
                                                             &using_fiber);
...
*/
    ...
    if (using_fiber) {
        ...
        // Don't need fiber for steal. Return it to the pool.
        START_INTERVAL(w, INTERVAL_FIBER_DEALLOCATE) {
            int ref_count = cilk_fiber_remove_reference(fiber, &w->l->fiber_pool);
            // Fibers we use when trying to steal should not be active,
            // and thus should not have any other references.
...
static
enum provably_good_steal_t
finish_provably_good_steal(__cilkrts_worker *w,
                           full_frame *ff) {
    ...
    if (NULL != ff->sync_master) {
        ...
        unset_sync_master(w->l->cached_team, ff);
        __cilkrts_push_next_frame(w->l->cached_team, ff);

        // If this is the team leader we're not abandoning the work
        if (w == w->l->cached_team)
                                               ...
                                               full_frame       *ff)
{
    ...
    START_INTERVAL(w, INTERVAL_PROVABLY_GOOD_STEAL) {
        if (decjoin(ff) == 0) {
            result = finish_provably_good_steal(w, ff);
        }
...
static
enum provably_good_steal_t pipe_iter_finish(__cilkrts_worker *w,
                                            full_frame *iter_ff)
{
    // ASSERT: we hold w->lock and ff->lock
    full_frame *parent_ff = iter_ff->parent;
    enum provably_good_steal_t result = ABANDON_EXECUTION;

    PIPER_DBG_PRINTF(1,
                     "W=%d: finishing pipe iter frame %p, control frame is %p\n",
                     w->self,
                     iter_ff, parent_ff);

#if 0
    // PLACEHOLDER for replay code.  Replay does not work with
    // pipelines yet.
    ...
    // Duh, the flag should say we have an pipe iteration on top when
    // when we are finishing a pipe iteration! :)
    CILK_ASSERT(w->l->implicit_deque_flags.iter_ff_on_top);

    START_INTERVAL(w, INTERVAL_PROVABLY_GOOD_STEAL) {
        if (decjoin(parent_ff) == 0) {

            CILK_ASSERT(!w->l->implicit_deque_flags.parent_resume);

            PIPER_DBG_PRINTF(1,
                             "W=%d: finishing pipe iter frame %p, control frame is %p. Decjoin went to 0?\n",
                             w->self,
                             iter_ff, parent_ff);

            // If the join counter on the parent frame has reached 0,
            // then the parent frame had better be be suspended.
            // We are about to resume at the sync.
            CILK_ASSERT(FF_PIPE_SUSPENDED_CONTROL != parent_ff->resume_flags);
            CILK_ASSERT(full_frame_is_suspended(parent_ff));
            result = finish_provably_good_steal(w, parent_ff);

            PIPER_DBG_PRINTF(1,
                             "W=%d: finish iter %p, should be resuming parent =%p. Implciit flag is set to %d\n",
                             w->self,
                             iter_ff, parent_ff,
                             w->l->implicit_deque_flags.parent_resume);

            // Once we finish with this iteration frame, clear the "next
            // iter" flag.
            ...
            // Finish iter_ff, and get the next frame to continue
            // executing.  Note that iter_ff will be destroyed after
            // this statement!
            full_frame *next_ff = piper_finish_iter_and_get_next(w, iter_ff);

            PIPER_DBG_PRINTF(3,
                             "W=%d: got next_frame %p to work on. iter_ff=%p, iter_ff->parent=%p\n",
                             w->self,
                             next_ff,
                             iter_ff,
                             iter_ff->parent);

            // In cases where we are resuming the parent control frame
            // after a self-steal, the parent will not have a fiber.
            if (next_ff) {
                possibly_fill_fiber_for_next_ff(w, next_ff);
                __cilkrts_push_next_suspended_frame(w, next_ff);
            }
            else {
                CILK_ASSERT(!w->l->implicit_deque_flags.parent_resume);
            ...
            PIPER_DBG_PRINTF(1,
                             "W=%d: finish iter %p, got next %p, implicit deque flags.parent_resume set to %d\n",
                             w->self,
                             iter_ff,
                             next_ff,
                             w->l->implicit_deque_flags.parent_resume);
...
static inline void
finish_setup_for_execution(__cilkrts_worker *w,
                           full_frame *ff,
                         ...
                         int is_return_from_call)
{
    ...
    switch (ff->resume_flags) {
        case FF_PIPE_RESUMING_CONTROL:
            ff->resume_flags = FF_NORMAL_CILK;
            piper_restore_worker_state_from_suspend_control(w,
                                                            ff->call_stack);
            break;
        case FF_PIPE_RESUMING_ITER:
            /* This path should not be called for resuming a pipeline iteration */
            CILK_ASSERT(0);
            break;
        default:
            // Normal return path.
            PIPER_DBG_PRINTF(2,
                             "W=%d: resuming ff =%p, callstack=%p, ff->resume_flags is %d\n",
                             w->self, ff, ff->call_stack, ff->resume_flags);
            setup_for_execution_reducers(w, ff);
            setup_for_execution_exceptions(w, ff);
    }
    finish_setup_for_execution(w, ff, is_return_from_call);
}

static void
piper_sched_setup_for_suspension(__cilkrts_worker *w,
                                 full_frame *suspending_ff,
                                 __cilkrts_stack_frame *active_sf)
{
    CILK_ASSERT(w->current_stack_frame == active_sf);

    // Mark the bottom frame in the chain as suspended.
    CILK_ASSERT(NULL == suspending_ff->call_stack);
    suspending_ff->call_stack = active_sf;
    active_sf->flags |= CILK_FRAME_SUSPENDED;  // Don't set the
    // stolen flag.
    __cilkrts_make_unrunnable_sysdep(w,
                                     suspending_ff,
                                     active_sf,
                                     0,
                                     "pipe_iter_suspend");
...
call_stack_get_unpromoted_parent(__cilkrts_stack_frame *sf)
{
    if (sf->flags & (CILK_FRAME_DETACHED|CILK_FRAME_STOLEN|CILK_FRAME_LAST)) {
        return NULL;
    }
    else {
        return sf->call_parent;
    }
}

static int
piper_sched_suspend_iteration_call_chain(__cilkrts_worker *w,
                                         full_frame *suspending_ff,
                                         __cilkrts_stack_frame *active_sf)
{
    int frames_suspended = 0;
    __cilkrts_stack_frame *current_sf = active_sf;

    // Walk up the call chain and NULL out the worker links for
    // all the frames linked in this call chain.
    do {
        current_sf->worker = NULL;
        frames_suspended++;
        current_sf = call_stack_get_unpromoted_parent(current_sf);
    } while (current_sf);

    if (frames_suspended > 2) {
        fprintf(stderr,
                "ERROR: W=%d suspending a call chain that is too long, starting at active_sf=%p\n",
                w->self,
                active_sf);

        // Count again, print out result, and crash.
        frames_suspended = 0;
        current_sf = active_sf;
        while (current_sf) {
            fprintf(stderr, "W=%d: chain elem %d = %p\n",
                    w->self,
                    frames_suspended,
                    current_sf);
            current_sf = current_sf->call_parent;
        }
    }

    PIPER_DBG_PRINTF(1,
                     "W=%d: suspend call chain. active_sf=%p, frames_suspended = %d\n",
                     w->self,
                     active_sf,
                     frames_suspended);
    return frames_suspended;
}

// Reverses the effects of piper_sched_suspend_call_chain.
static
__cilkrts_stack_frame*
piper_sched_resume_iteration_call_chain(__cilkrts_worker *w,
                                        full_frame *ff_to_resume)
{
    __cilkrts_stack_frame* active_sf = ff_to_resume->call_stack;
    int num_in_chain = 1;
    __cilkrts_stack_frame *sf = call_stack_get_unpromoted_parent(active_sf);

    while (sf) {
        CILK_ASSERT(NULL == sf->worker);
        sf->worker = w;
        num_in_chain++;
        sf = call_stack_get_unpromoted_parent(sf);
    }

    CILK_ASSERT(num_in_chain <= 2);
    return active_sf;
}
...
static
full_frame*
piper_sched_iter_setup_for_execution(__cilkrts_worker *w,
                                     full_frame *iter_ff,
                                     __cilkrts_stack_frame** sf_to_resume)
{
    // ASSERT: We own w->lock and iter_ff->lock || P == 1
    full_frame *ff_to_resume = NULL;
    CILK_ASSERT(FF_PIPE_RESUMING_ITER == iter_ff->resume_flags);

    // Set the flags back on the iteration iter_ff.
    iter_ff->resume_flags = FF_NORMAL_CILK;

    // Figure out which full frame we are actually resuming.
    // (The frame should be either iter_ff, or one of its
    //  direct call children).
    ff_to_resume =
            piper_restore_worker_state_from_suspend_iter(w, iter_ff);

    // Put the rest of the call chain back into a useful state.
    *sf_to_resume = piper_sched_resume_iteration_call_chain(w, ff_to_resume);
    CILK_ASSERT(*sf_to_resume);

    PIPER_DBG_PRINTF(1,
                     "W=%d: resume iter: iter_ff = %p, ff_to_resume=%p, sf=%p\n",
                     w->self,
                     iter_ff,
                     ff_to_resume,
                     *sf_to_resume);

    if (ff_to_resume != iter_ff) {
        CILK_ASSERT(ff_to_resume->parent == iter_ff);
        CILK_ASSERT(ff_to_resume->is_call_child);
        BEGIN_WITH_FRAME_LOCK(w, ff_to_resume) {
                finish_setup_for_execution(w, ff_to_resume, 0);
            } END_WITH_FRAME_LOCK(w, ff_to_resume);
    }
    else {
        finish_setup_for_execution(w, iter_ff, 0);
    }

    PIPER_DBG_PRINTF(1,
                     "W=%d: resume_iter: iter_ff=%p sf=%p\n",
                     w->self,
                     iter_ff,
                     *sf_to_resume);
...
full_frame* check_for_work(__cilkrts_worker *w, steal_t s)
{
    ...
    if (NULL == ff) {
        //unsigned begin, end, begin2, end2;
        START_INTERVAL(w, INTERVAL_STEALING) {
            if (w->l->type != WORKER_USER && w->l->batch_id == -1 && w->l->cached_team != NULL) {
                ...
                w->l->team = NULL;
                w->l->cached_team = NULL;
                __cilkrts_worker_unlock(w);
            }
            //begin = beginCycleCount();
        ...
        if (NULL == ff) {
            ...
            // No quantum for you!
            //w->l->idle_time += (end-begin);
            //begin2 = beginCycleCount();
            __cilkrts_yield();
            //end2 = endCycleCount();
            //w->l->idle_time += (end2-begin2);
                                               ...
                                               void* wptr)
{
    ...
    BEGIN_WITH_WORKER_LOCK(w) {
        ...
        BEGIN_WITH_FRAME_LOCK(w, ff) {
                // If we are resuming a pipeline iteration, then
                // the iteration stack frame may still have a
                // valid call chain associated with it (i.e.,
                // at least a pointer to its spawn helper).
                is_pipe_iter = (FF_PIPE_RESUMING_ITER == ff->resume_flags);
                if (is_pipe_iter) {
                    ff_to_resume = piper_sched_iter_setup_for_execution(w, ff, &sf);
                }
                else {
                    // Normal return to resume execution of a full frame.
                    ff_to_resume = ff;
                    sf = ff->call_stack;
    ...
    if (!is_pipe_iter) {
        scheduling_fiber_prepare_to_resume_user_code(w, ff_to_resume, sf);
    }
...
static void
piper_sched_mark_suspended_control_frame_as_resumed(__cilkrts_worker *w,
                                                    full_frame *ff)
{
    // To resume execution of a frame, just push it
    // on to the one-element queue of the current worker, and then
    // jump into the scheduling loop.
    __cilkrts_push_next_suspended_frame(w, ff);

    PIPER_DBG_PRINTF(2,
                     "W=%d: pushing suspended control frame ff=%p to resume...\n",
                     w->self, ff);
}

static void do_suspend_pipe_control_frame(__cilkrts_worker *w,
                                          full_frame *ff,
                                          __cilkrts_stack_frame *sf)
{
    PIPER_DBG_PRINTF(2,
                     "w->self = %d: suspends current frame sf=%p. ff=%p, fiber=%p\n",
                     w->self,
                     sf,
                     ff,
                     ff->fiber_self);
    ...
    // Flag to indicate whether failed at suspending this frame.
    // Assume successful suspension for now.
    int failed_suspend = 0;

    BEGIN_WITH_WORKER_LOCK_OPTIONAL(w) {
        CILK_ASSERT(ff);
        BEGIN_WITH_FRAME_LOCK(w, ff) {
                // Before suspending the full frame, the call stack should
                // be NULL.
                CILK_ASSERT(!full_frame_is_suspended(ff));
                piper_save_worker_state_for_suspend_control(w, sf);

                make_unrunnable(w, ff, sf, 0, "pipe_frame_suspend");

                // After the full frame is suspended, call_stack is
                // non-null.  The runtime will check this field
                // later, to figure out whether it can resume execution.
                CILK_ASSERT(full_frame_is_suspended(ff));
                CILK_ASSERT(sf == ff->call_stack);
                CILK_ASSERT(NULL == *w->l->frame_ff);

                // Check for real whether we need to suspend the control
                // frame or not.  We did a preliminary check without
                // holding the control frame lock to get here.  But we
                // need to verify it now that we hold the lock, to make
                // sure this control frame never gets dropped on the
                // floor.
                int suspended =
                        piper_try_suspend_control_frame_at_throttle(w,
                                                                    ff,
                                                                    sf);

                if (!suspended) {
                    // If we didn't really need to suspend, then just push
                    // the full frame back on our one-element queue.
                    //
                    // TBD: It seems like it would be slightly faster to
                    // do this check before actually going through the
                    // bookkeeping of suspending the current frame.
                    // But maybe it isn't really worth it.

                    PIPER_DBG_PRINTF(1,
                                     "W=%d: Ok, didn't really mean to suspend control ff=%p, sf=%p. pushing it back\n",
                                     w->self,
                                     ff, sf);
                    piper_sched_mark_suspended_control_frame_as_resumed(w, ff);
                    failed_suspend = 1;
                }
            } END_WITH_FRAME_LOCK(w, ff);
    } END_WITH_WORKER_LOCK_OPTIONAL(w);
    ...
    PIPER_DBG_PRINTF(1,
                     "W=%d: Jumping to scheduling loop after suspend of ff=%p, sf=%p.... ff->call_stack=%p, failed_suspend=%d Parent resume is %d, next_frame_ff=%p\n",
                     w->self,
                     ff,
                     sf,
                     ff->call_stack,
...
static void do_suspend_pipe_iter_frame(__cilkrts_worker *w,
                                       full_frame *current_ff,
                                       __cilkrts_stack_frame *active_sf)
{
    // Figure out whether we are suspending at iteration directly,
    // or if we are suspending at one level nested in the call chain.
    full_frame *iter_ff;
    if (FF_PIPE_ITER_CALL_DESCENDANT == current_ff->frame_type) {
        iter_ff = current_ff->parent;
    }
    else {
        iter_ff = current_ff;
    }
    ...
    PIPER_DBG_PRINTF(2,
                     "w->self = %d: suspends for sf=%p,  current_ff=%p, iter_ff=%p, fiber=%p\n",
                     w->self,
                     active_sf,
                     current_ff,
                     iter_ff,
                     current_ff->fiber_self);
    CILK_ASSERT(active_sf->worker == w);
    ...
    BEGIN_WITH_WORKER_LOCK(w) {
        BEGIN_WITH_FRAME_LOCK(w, iter_ff) {

                // Before suspending the full frame, the call stack should
                // be NULL.
                CILK_ASSERT(!full_frame_is_suspended(current_ff));
                piper_save_worker_state_for_suspend_iter(w, iter_ff, current_ff);

                CILK_ASSERT(active_sf == w->current_stack_frame);

                // Setup the frame at the bottom of the call chain to be
                // suspended.
                piper_sched_setup_for_suspension(w, current_ff, active_sf);

                // Do any suspension tasks needed for the call chain.
                piper_sched_suspend_iteration_call_chain(w, current_ff, active_sf);

                // After the full frame is suspended, call_stack is
                // non-null.  The runtime will check this field
                // later, to figure out whether it can resume execution.
                CILK_ASSERT(full_frame_is_suspended(current_ff));
                CILK_ASSERT(active_sf == current_ff->call_stack);
                CILK_ASSERT(NULL == *w->l->frame_ff);

                full_frame* next_ff = piper_try_suspend_at_stage_wait(w, iter_ff);
                PIPER_DBG_PRINTF(1,
                                 "W=%d: ok, tried to suspend stage active_sf=%p, current_ff=%p, iter_ff=%p... current_ff->call_stack=%p, Got next_ff=%p\n",
                                 w->self,
                                 active_sf, current_ff,
                                 iter_ff,
                                 current_ff->call_stack,
                                 next_ff);

                if (next_ff) {
                    int needed_fiber = possibly_fill_fiber_for_next_ff(w, next_ff);
                    if (iter_ff == next_ff) {
                        failed_suspend = 1;
                        CILK_ASSERT(!needed_fiber);
                    }
                    __cilkrts_push_next_suspended_frame(w, next_ff);
                }
    ...
    PIPER_DBG_PRINTF(1,
                     "W=%d: Jumping to scheduling loop after suspend of current_ff=%p, active_sf=%p (iter_ff=%p) ... current_ff->call_stack=%p, failed_suspend=%d Parent resume is %d, next_frame_ff=%p\n",
                     w->self,
                     current_ff, active_sf,
                     iter_ff,
                     current_ff->call_stack,
                     failed_suspend,
                     w->l->implicit_deque_flags.parent_resume,
...
__cilkrts_worker*
piper_sched_suspend_pipe_iter_frame(__cilkrts_worker *w,
                                    full_frame *iter_ff,
                                    __cilkrts_stack_frame *active_sf)
{
    __cilkrts_pipe_iter_data* iter_data_dbg
            = piper_get_iter_data_for_ff(iter_ff);
    CILK_ASSERT(iter_data_dbg);

    PIPER_DBG_PRINTF(1,
                     "W=%d: suspending iter with iter->sf=%p,  active_sf %p, with control frame %p\n",
                     w->self,
                     iter_data_dbg->sf,
                     active_sf,
                     iter_data_dbg->control_sf);

    w->l->post_suspend = do_suspend_pipe_iter_frame;
    w->l->suspended_stack = active_sf;

    ITT_SYNC_RELEASING(w);
    ITT_SYNC_PREPARE(w);

    full_frame *current_ff = *w->l->frame_ff;
    cilk_fiber *current_fiber = current_ff->fiber_self;

    CILK_ASSERT((iter_ff == current_ff) ||
                (iter_ff == current_ff->parent));

    PIPER_DBG_PRINTF(1,
                     "ThreadId=%p, W=%d: suspending on pipe iteration %p. fiber=%p. ff->child_red_map=%p, w->reducer_map=%p\n",
                     cilkos_get_current_thread_id(),
                     w->self,
                     iter_ff,
                     current_fiber,
                     iter_ff->children_reducer_map,
                     w->reducer_map);

    PIPER_DBG_PRINTF(1,
                     "W=%d: iter_sf=%p, control_sf=%p\n",
                     w->self,
                     iter_data_dbg->sf, iter_data_dbg->control_sf);

    cilk_fiber_data* fdata = cilk_fiber_get_data(current_fiber);

    PIPER_DBG_PRINTF(1,
                     "ThreadId=%p, W=%d. suspend pipe_iter %p, fiber=%p, resume sf was %p, will set to NULL, iter_ff->call_stack=%p\n",
                     cilkos_get_current_thread_id(),
                     w->self,
                     iter_ff,
                     current_fiber,
...
#if PIPER_DEBUG >= 1
    {
        __cilkrts_worker* wkr = __cilkrts_get_tls_worker();
        __cilkrts_stack_frame* c0 = wkr->current_stack_frame;
        __cilkrts_stack_frame* c1 = c0->call_parent;
        __cilkrts_stack_frame* c2 = c1 ? c1->call_parent : NULL;
        PIPER_DBG_PRINTF(1,
                         "W=%d, wkr=%d: current=%p, parent=%p, gparent=%p\n",
                         w->self, wkr->self, c0, c1, c2);
    }
#endif // PIPER_DEBUG >= 1
...
#if FIBER_DEBUG >= 2
    fprintf(stderr, "ThreadId=%p, W=%d: about to suspend self into runtime... current_fiber = %p, switch to fiber %p\n",
            cilkos_get_current_thread_id(),
            w->self,
            current_fiber, w->l->scheduling_fiber);
#endif
    ...
    // After we resume, reset our worker.

    PIPER_DBG_PRINTF(1,
                     "W=%d.  active_sf = %p. about to resume... setting worker\n",
                     fdata->owner->self, active_sf);
    CILK_ASSERT(fdata->owner->current_stack_frame == active_sf);
    active_sf->worker = fdata->owner;
    ITT_SYNC_ACQUIRED(fdata->owner);
    return fdata->owner;
}


__cilkrts_worker*
piper_sched_suspend_pipe_control_frame(__cilkrts_worker *w,
                                       full_frame *control_ff,
                                       __cilkrts_stack_frame *control_sf)
{
    // This function does not do anything yet but wait for a bit.
    CILK_ASSERT(control_ff == *w->l->frame_ff);

    w->l->post_suspend = do_suspend_pipe_control_frame;
    w->l->suspended_stack = control_sf;
    ...
    cilk_fiber *current_fiber = control_ff->fiber_self;

    PIPER_DBG_PRINTF(1,
                     "ThreadId=%p, W=%d: suspending on pipe full frame %p. fiber=%p. ff->child_red_map=%p, w->reducer_map=%p\n",
                     cilkos_get_current_thread_id(),
                     w->self,
                     control_ff,
                     control_ff->fiber_self,
                     control_ff->children_reducer_map,
                     w->reducer_map);
    ...
    PIPER_DBG_PRINTF(1,
                     "W=%d.  active_sf = %p. about to resume... setting worker\n",
                     fdata->owner->self, control_sf);
    CILK_ASSERT(fdata->owner->current_stack_frame == control_sf);
    CILK_ASSERT(control_sf->worker == fdata->owner);

    if (PIPER_DEBUG >= 0) {
        CILK_ASSERT(control_sf->worker == __cilkrts_get_tls_worker());
    }
...
// Should be called while holding the lock on w.
void piper_sched_promote_rightmost_iter(__cilkrts_worker *w,
                                        __cilkrts_pipe_iter_data* iter_data)
{
    __cilkrts_stack_frame *iter_sf = iter_data->sf;
    __cilkrts_stack_frame *control_sf = &(iter_data->control_sf->sf);

    // Otherwise, we actually need to do the promotion.
    //assert(dekker_protocol(w));

    // If a steal hasn't happened, then we better be executing
    // in the rightmost iteration, and we should have exactly
    // one stealable element on our deque --- the control
    // frame.

    // The rest of this code mimics "detach_for_steal",
    // except it isn't a simulated steal.
    //
    // TBD: There is probably a better way to reuse code,
    // but I want to make sure this code works first...
    full_frame *control_ff = *w->l->frame_ff;

    // Full frame we are stealing away.
    // In this case, it should be control_ff.
    full_frame *loot_ff;

    // The new child iteration frame we are installing on w.
    full_frame *iter_ff;

    __cilkrts_stack_frame *volatile *h = *w->head;
    // Remove the one element from the deque.
    CILK_ASSERT((size_t)(*w->tail - *w->head) == 1);
    *w->head = h + 1;

#if PIPER_DEBUG >= 1
    {
        __cilkrts_stack_frame *c0 = w->current_stack_frame;
        __cilkrts_stack_frame *c1 = c0 ? c0->call_parent : NULL;
        __cilkrts_stack_frame *c2 = c1 ? c1->call_parent : NULL;
        PIPER_DBG_PRINTF(1,
                         "W=%d: unroll call stack. c=%p, parent=%p, gparent=%p. control_sf=%p, iter_data->sf=%p\n",
                         w->self, c0, c1, c2, control_sf, iter_data->sf);
    }
#endif

    BEGIN_WITH_FRAME_LOCK(w, control_ff) {
            // If this were a normal steal, we would decrement the
            // join counter on control_ff here, and then increment
            // it again when we push it onto a one-element queue
            // to resume it.  But since the rightmost child will
            // conceptually hang onto  the pointer to the parent and
            // the steal is implicit, we leave the join counter of
            // the parent alone.

            /* The call stack should be the control frame. */
            CILK_ASSERT((*h) == control_sf);

            // Check call parent of the iteration's spawn helper.
            CILK_ASSERT(iter_sf->call_parent == control_sf);

            /* unroll the call stack to steal control_ff away. */
            loot_ff = unroll_call_stack(w, control_ff, control_sf);
            // We expect to be stealing the control frame!
            CILK_ASSERT(loot_ff == control_ff);
            ...
            if (WORKER_USER == w->l->type &&
                ...
                // This call is a shared access to
                // victim->l->last_full_frame.
                set_sync_master(w, loot_ff);
            }

            // Don't mark this frame as simulated stolen though.
            loot_ff->call_stack->flags |= CILK_FRAME_UNSYNCHED;

            CILK_ASSERT(control_sf->flags & CILK_FRAME_PIPER);

            PIPER_DBG_PRINTF(1,
                             "W=%d: just promoted our own control frame: control_sf = %p, loot_ff=%p, loot_ff->call_stack=%p\n",
                             w->self, control_sf, loot_ff, loot_ff->call_stack);

            iter_ff = make_pipe_iter_child(w,
                                           loot_ff,
                                           control_sf,
                                           PLACEHOLDER_FIBER);

            // TBD: Do we actually need this part? Shouldn't?
            // We are going to break the link between the
            // iteration spawn helper and the control frame.
            // The installation of the control frame should break
            // the link.
            PIPER_DBG_PRINTF(1,
                             "W=%d: in promote_right: iter_sf = %p, iter_sf->call_parent is %p\n",
                             w->self, iter_sf, iter_sf->call_parent);
            iter_sf->call_parent = NULL;
            ...
            BEGIN_WITH_FRAME_LOCK(w, iter_ff) {
                    ...
                    piper_install_iteration_frame(w,
                                                  ...
                                                  iter_ff,
                                                  w);
                } END_WITH_FRAME_LOCK(w, iter_ff);
        } END_WITH_FRAME_LOCK(w, control_ff);
}
                                ...
                                __cilkrts_stack_frame *returning_sf)
{
    ...
    if (stolen_p)
    {
        ...
        longjmp_into_runtime(w, do_return_from_spawn, saved_sf);
                                 ...
                                 __cilkrts_stack_frame *sf)
{
    full_frame *parent_ff;
    enum provably_good_steal_t steal_result = ABANDON_EXECUTION;
    int is_pipe_iter_frame = (ff->frame_type == FF_PIPE_ITER);
    ...
    BEGIN_WITH_WORKER_LOCK_OPTIONAL(w) {
        ...
        BEGIN_WITH_FRAME_LOCK(w, parent_ff) {
            if (parent_ff->simulated_stolen) {
                if (FF_DEFAULT_TYPE == ff->frame_type) {
                    CILK_ASSERT(worker_implicit_steal_flags_cleared(w));
                }
                else {
                    CILK_ASSERT(FF_PIPE_ITER_SIMULATED == ff->frame_type);
                    // We need to call post-leave-frame code, as
                    // though this frame hadn't been stolen.
                    CILK_ASSERT(sf != NULL);
                    piper_leave_iter_frame_for_simulated_steal(sf);
                }
                // Now continue with the parent as usual.
                unconditional_steal(w, parent_ff);
            }
            else if (!is_pipe_iter_frame) {
                //CILK_ASSERT(worker_implicit_steal_flags_cleared(w));
                steal_result = provably_good_steal(w, parent_ff);
            }
            else {
                steal_result = pipe_iter_finish(w, ff);
            }
        } END_WITH_FRAME_LOCK(w, parent_ff);
...
void __cilkrts_deinit_internal(global_state_t *g)
{
    ...
    if (g->pin_map) {
        pinning_destroy_system_map(g->pin_map);
    }
...
void __cilkrts_init_internal(int start)
{
    ...
    if (cilkg_is_published()) {
        ...
        if (cilkg_is_published()) {
            ...
            // Read in system cpu map.
            if (g->pin_options.policy != PIN_NONE) {
                g->pin_map = pinning_create_system_map(&g->pin_options,
                                                       g->pin_options.verbose);
            }
