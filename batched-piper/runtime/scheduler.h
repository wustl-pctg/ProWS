...
/**
 * @brief Suspend execution of a control frame for a pipe_while loop.
 *
 * This method returns after the control frame has been resumed.
 *
 * @param w           The worker currently executing the control frame.
 * @param control_ff  The full frame for the control frame.
 * @param control_sf  The pipe_control_frame_t* for the control frame.
 * @return            The worker we resume execution on.
 */
COMMON_PORTABLE
__cilkrts_worker*
...
/**
 * @brief Suspend and resume execution of an iteration frame for a
 * pipe_while loop.
 *
 * This method switches stacks into the runtime to suspend execution
 * of a pipeline iteration, and eventually returns (possibly on a
 * different worker).
 *
 * @param w          The worker currently executing the control frame.
 * @param iter_ff    The full frame for the iteration.
 * @param active_sf  The stack frame that @c w is currently executing.
 * @return           The worker we resume execution on.
 */
COMMON_PORTABLE
__cilkrts_worker*
...
/**
 * @brief Try to self-steal the control frame from w's deque while it
 *  is (trying) to execute the rightmost pipeline iteration.
 *
 * This method should be called while holding w's deque lock.
 *
 * After this method finishes, the current iteration (in the pipe
 * control frame) has now been promoted, i.e., its full frame is
 * valid.
 *
 * Also, w->l->implicit_deque_flags.parent_resume is set to true, to
 * indicate that the parent (control frame) can be resumed.
 */
COMMON_PORTABLE
void
		piper_sched_promote_rightmost_iter(__cilkrts_worker *w,
