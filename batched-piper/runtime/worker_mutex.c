...
// Acquire a mutex, except use the hardware transactional mechanisms.
void __cilkrts_mutex_lock_speculative(__cilkrts_worker *w, struct mutex *m)
{
  // On an abort, or if we have no hardware transactional
  // mechanisms, we fall through and execute
  // the normal locking code.
  __cilkrts_mutex_lock(w, m);
}

int __cilkrts_mutex_trylock_speculative(__cilkrts_worker *w, struct mutex *m)
{
  return __cilkrts_mutex_trylock(w, m);
}

void __cilkrts_mutex_unlock_speculative(__cilkrts_worker *w, struct mutex *m)
{
  {
    // Otherwise, we took the explicit-acquire path.
    __cilkrts_mutex_unlock(w, m);
  }
}
