...
static int create_output_file(char *outfile) {
    ...
    WHEN_TIMING({
        ...
        t1 = ktiming_diff_nsec(&begin, &end);
        begin = end;
    })
    ...
    WHEN_TIMING({
        ...
        t2 = ktiming_diff_nsec(&begin, &end);
...
void Encode(config_t * _conf) {
...
#if VIEWREAD
    cilk_set_reducer(args.f_out_reducer, __builtin_return_address(0), __FUNCTION__, __LINE__);
#endif
    ...
    if(conf->preloading) {
        ...
        preload_time = ktiming_diff_nsec(&begin, &preload_end);
    ...
    process_time = ktiming_diff_nsec(&begin, &end);

#if VIEWREAD
    cilk_read_reducer(args.f_out_reducer, __builtin_return_address(0), __FUNCTION__, __LINE__);
#endif
