...
#include <cilksan.h>
#else
#define BEGIN_UPDATE_STRAND
#define END_UPDATE_STRAND
#endif

/* static FILE *fout; */
static std::ostream *fout;
static cilk::reducer_ostream *fout_reducer;
...
#if VIEWREAD
    cilk_set_reducer(fout_reducer, __builtin_return_address(0), __FUNCTION__, __LINE__);
#endif
    scan(query_dir);

#if VIEWREAD
    cilk_read_reducer(fout_reducer, __builtin_return_address(0), __FUNCTION__, __LINE__);
#endif

    fb.close();
    cilk::aligned_delete(fout_reducer);

    stimer_tuck(&tmr, "Processing time");
