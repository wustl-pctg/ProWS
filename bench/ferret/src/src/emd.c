    ...
    cass_size_t dim, void *param, flow_t *Flow, int*FlowSize)
{
  ...
  emd_node1_t *U = malloc(sizeof(emd_node1_t) * MAX_SIG_SIZE1);
  emd_node1_t *V = malloc(sizeof(emd_node1_t) * MAX_SIG_SIZE1);
...
emdinit(emd_state_t *state, signature_t *Signature1, signature_t *Signature2, float (*Dist)(cass_size_t, feature_t, feature_t, void *), cass_size_t dim, void *param)
{
  ...
  double *S = malloc(sizeof(double) * MAX_SIG_SIZE1);
  double *D = malloc(sizeof(double) * MAX_SIG_SIZE1);
...
newSol(emd_state_t *state)
{
  ...
  emd_node2_t *CurX, *LeaveX;
  emd_node2_t **Loop = malloc(sizeof(emd_node2_t*) * 2*MAX_SIG_SIZE1);
...
findLoop(emd_state_t *state, emd_node2_t **Loop)
{
  ...
  char *IsUsed = malloc(sizeof(char) * 2*MAX_SIG_SIZE1);
...
russel(emd_state_t *state, double *S, double *D)
{
  ...
  double **Delta = malloc(sizeof(double*) * MAX_SIG_SIZE1);
  for (int i = 0; i < MAX_SIG_SIZE1; ++i)
    Delta[i] = malloc(sizeof(double) * MAX_SIG_SIZE1);
  emd_node1_t *Ur = malloc(sizeof(emd_node1_t) * MAX_SIG_SIZE1);
  emd_node1_t *Vr = malloc(sizeof(emd_node1_t) * MAX_SIG_SIZE1);
