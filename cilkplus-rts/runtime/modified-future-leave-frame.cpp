...
void finish_spawn_return_on_user_stack(__cilkrts_worker *w,
    ...
    if (child_ff->future_flags == CILK_FUTURE) {
        w->l->fiber_to_free = child_ff->fiber_self;
        //parent_ff->fiber_child = child_ff->fiber_self;
    }
    // Execute left-holder logic for stacks.
    else if (child_ff->left_sibling || (parent_ff->fiber_child && parent_ff->fiber_child != child_ff->fiber_self)) {
        ...
        CILK_ASSERT(! "We set the fiber child though!?");
...
CILK_ABI_VOID __cilkrts_leave_future_frame(__cilkrts_stack_frame *sf) {
    ...
    if ((sf->flags & CILK_FRAME_DETACHED)) {
        ...
        if (__builtin_expect(sf->flags & CILK_FRAME_EXCEPTING, 0)) {
            CILK_ASSERT(0); // KYLE_TODO: should not be down this path yet
    ...
    CILK_ASSERT(0); // KYLE_TODO: Should not reach here; this is the future!
