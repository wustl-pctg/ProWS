...
static int NTHREAD_LOAD = 1;
static int NTHREAD_SEG	= 1;
static int NTHREAD_EXTRACT = 1;
static int NTHREAD_VEC	= 1;
static int NTHREAD_RANK = 1;
static int NTHREAD_OUT	= 1;
...
static struct queue q_load_seg;
...
static struct queue q_seg_extract;
...
static struct queue q_extract_vec;
...
static struct queue q_vec_rank;
...
static struct queue q_rank_out;
...
void *t_seg (void *dummy)
{
	...
	while(1)
	{
		if(dequeue(&q_load_seg, (void **)&load) < 0)
		    break;
...
void *t_extract (void *dummy)
{
	...
	while (1)
	{
		if(dequeue(&q_seg_extract, (void **)&seg) < 0)
		    break;
...
void *t_vec (void *dummy)
{
	...
	while(1)
	{
		if(dequeue(&q_extract_vec, (void **)&extract) < 0)
		    break;
...
void *t_rank (void *dummy)
{
	...
	while (1)
	{
		if(dequeue(&q_vec_rank, (void **)&vec) < 0)
		    break;
...
void *t_out (void *dummy)
{
	...
	while (1)
	{
		if(dequeue(&q_rank_out, (void **)&rank) < 0)
		    break;
...
int main (int argc, char *argv[]) {
	...
	t_load_desc[0].arg = (void *)query_dir;
