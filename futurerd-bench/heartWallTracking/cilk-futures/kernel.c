...
void kernel(public_struct public, private_struct private) {
    ...
    if(public.frame_no == 0) {
        // ANGE: compute_part1: if(frame_no == 0) kernel_part_start 
        // update temporary row/col coordinates
        int pointer = private.point_no*public.frames+public.frame_no;
        ...
        fp *d_in = &private.d_T[private.in_pointer];

        // update template, limit the number of working threads to the size of template
        for(int col=0; col<public.in_mod_cols; col++) {
            for(int row=0; row<public.in_mod_rows; row++) {
                ...
                int ori_row = private.d_Row[private.point_no] - 25 + row - 1;
                int ori_col = private.d_Col[private.point_no] - 25 + col - 1;
                int ori_pointer = ori_col*public.frame_rows+ori_row;
                // update template
    ...
    if(public.frame_no != 0) {
        ...
        int in2_rowlow = private.d_Row[private.point_no] - public.sSize; // (1 to n+1)
        int in2_collow = private.d_Col[private.point_no] - public.sSize;

        // work
        for(int col=0; col<public.in2_cols; col++) {
            for(int row=0; row<public.in2_rows; row++) {
                ...
                fp temp = public.d_frame[ori_col*public.frame_rows+ori_row];
        ...
        // variables
        // ANGE: compute_part2: if(frame_no != 0) kenel_part2
        fp *d_in = &private.d_T[private.in_pointer];
        ...
        for(int col=0; col<public.in_mod_cols; col++) {
            for(int row=0; row<public.in_mod_rows; row++) {
                ...
                int rot_col = (public.in_mod_rows-1) - col;
        ...
        fp mean = in_final_sum / public.in_mod_elem; // gets mean (average) value of element in ROI
        fp mean_sqr = mean * mean;
        fp variance  = (in_sqr_final_sum / public.in_mod_elem) - mean_sqr; // gets variance of ROI
        ...
        // ANGE: compute_part4: if(frame_no != 0) kenel_part4
        // work
        for(int col=1; col<=public.conv_cols; col++) {
            // column setup
            int j = col + public.joffset;
            int jp1 = j + 1;
            int ja1, ja2;
            if(public.in2_cols < jp1) { ja1 = jp1 - public.in2_cols; }
            else{ ja1 = 1; }

            if(public.in_mod_cols < j) { ja2 = public.in_mod_cols; }
            else{ ja2 = j; }

            for(int row=1; row<=public.conv_rows; row++) {
                // row range setup
                int i = row + public.ioffset;
                int ip1 = i + 1;
                int ia1, ia2;

                if(public.in2_rows < ip1) { ia1 = ip1 - public.in2_rows; }
                else{ ia1 = 1; }

                if(public.in_mod_rows < i) { ia2 = public.in_mod_rows; }
                else{ ia2 = i; }
        ...
        // ANGE: compute_part5: if(frame_no != 0) kenel_part5
        // work
        for(int col=0; col<public.in2_pad_cols; col++) {
            for(int row=0; row<public.in2_pad_rows; row++) {
                // execution
                if(row > (public.in2_pad_add_rows-1) && // do if has numbers in original array
                    ...
                    int ori_row = row - public.in2_pad_add_rows;
                    int ori_col = col - public.in2_pad_add_cols;
        ...
        for(int ei_new = 0; ei_new < public.in2_pad_cols; ei_new++) {
            // figure out column position
            int pos_ori = ei_new*public.in2_pad_rows;
            // loop through all rows
            int sum = 0;
            for(int position = pos_ori; position < pos_ori+public.in2_pad_rows; position = position + 1) {
        ...
        // work
        for(int col=0; col<public.in2_sub_cols; col++) {
            for(int row=0; row<public.in2_sub_rows; row++) {
                // figure out corresponding location in old matrix and copy values to new matrix
                int ori_row = row + public.in2_pad_cumv_sel_rowlow - 1;
                int ori_col = col + public.in2_pad_cumv_sel_collow - 1;
                fp temp = private.d_in2_pad[ori_col*public.in2_pad_rows+ori_row];
                // figure out corresponding location in old matrix and copy values to new matrix
                ori_row = row + public.in2_pad_cumv_sel2_rowlow - 1;
                ori_col = col + public.in2_pad_cumv_sel2_collow - 1;
                fp temp2 = private.d_in2_pad[ori_col*public.in2_pad_rows+ori_row];
                // subtraction
        ...
        // ANGE: compute_part6: if(frame_no != 0) kenel_part6
        for(int ei_new = 0; ei_new < public.in2_sub_rows; ei_new++) {
            // figure out row position
            int pos_ori = ei_new;
            // loop through all rows
            int sum = 0;
            for(int position = pos_ori; 
                    position < pos_ori+public.in2_sub_elem; 
        ...
        // work
        for(int col=0; col<public.in2_sub2_sqr_cols; col++) {
            for(int row=0; row<public.in2_sub2_sqr_rows; row++) {
                // figure out corresponding location in old matrix and copy values to new matrix
                int ori_row = row + public.in2_sub_cumh_sel_rowlow - 1;
                int ori_col = col + public.in2_sub_cumh_sel_collow - 1;
                fp temp = private.d_in2_sub[ori_col*public.in2_sub_rows+ori_row];

                // figure out corresponding location in old matrix and copy values to new matrix
                ori_row = row + public.in2_sub_cumh_sel2_rowlow - 1;
                ori_col = col + public.in2_sub_cumh_sel2_collow - 1;
                fp temp2 = private.d_in2_sub[ori_col*public.in2_sub_rows+ori_row];
                // subtraction
                temp2 = temp - temp2;
                // squaring
        ...
        // ANGE: compute_part7: if(frame_no != 0) kenel_part7
        // work
        for(int col=0; col<public.in2_pad_cols; col++) {
            for(int row=0; row<public.in2_pad_rows; row++) {
                // execution
        ...
        // work
        for(int col=0; col<public.conv_cols; col++) {
            for(int row=0; row<public.conv_rows; row++) {
                // figure out corresponding location in old matrix and copy values to new matrix
                int ori_row = row + public.in2_sub_cumh_sel_rowlow - 1;
                int ori_col = col + public.in2_sub_cumh_sel_collow - 1;
                fp temp = private.d_in2_sub[ori_col*public.in2_sub_rows+ori_row];
        ...
        // ANGE: compute_part9: if(frame_no != 0) kenel_part9
        // parameters
        int cent = public.sSize + public.tSize + 1;
        int pointer = public.frame_no-1+private.point_no*public.frames;
        int tMask_row = cent + private.d_tRowLoc[pointer] - private.d_Row[private.point_no] - 1;
        int tMask_col = cent + private.d_tColLoc[pointer] - private.d_Col[private.point_no] - 1;

        // work
        for(int ei_new = 0; ei_new < public.tMask_elem; ei_new++) {
        ...
        // ANGE: compute_part10: if(frame_no != 0) kenel_part10
        // work
        for(int col=1; col<=public.mask_conv_cols; col++) {
            // col setup
            int j = col + public.mask_conv_joffset;
            int jp1 = j + 1;
            int ja1, ja2;
            if(public.mask_cols < jp1) { ja1 = jp1 - public.mask_cols; }
            else{ ja1 = 1; }

            if(public.tMask_cols < j) { ja2 = public.tMask_cols; }
            else{ ja2 = j; }

            for(int row=1; row<=public.mask_conv_rows; row++) {
                // row setup
                int i = row + public.mask_conv_ioffset;
                int ip1 = i + 1;
                int ia1, ia2;

                if(public.mask_rows < ip1) { ia1 = ip1 - public.mask_rows; }
                else{ ia1 = 1; }
                if(public.tMask_rows < i) { ia2 = public.tMask_rows; }
                else{ ia2 = i; }
        ...
        fp fin_max_val = 0;
        int fin_max_coo = 0;
        for(int i=0; i<public.mask_conv_elem; i++) {
        ...
        int largest_row = (fin_max_coo+1) % public.mask_conv_rows - 1; // (0-n) row
        int largest_col = (fin_max_coo+1) / public.mask_conv_rows; // (0-n) column
        ...
        int offset_row = largest_row - public.in_mod_rows - (public.sSize - public.tSize);
        int offset_col = largest_col - public.in_mod_cols - (public.sSize - public.tSize);
        // ANGE: the next line is commented out from kernel_part10
        // ANGE: it is HWTcompute_part0, which is also called in
        //       compute_part10 for all frames except the last one 
        ...
        // ANGE: end of, if(frame_no == 0) kernel_part_start 
    }
    ...
    // ANGE: kernel_part_stop, as part of HWMcompute_part10
    ...
    if(public.frame_no != 0 && (public.frame_no)%10 == 0) {
        ...
        // update coordinate
        int loc_pointer = private.point_no*public.frames+public.frame_no;
