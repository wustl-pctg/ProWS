...
template<class T, class T2>
bool FlexFilterRowVTBB(FlexImage<T,1> &src, FlexImage<T,1> &dst, 
                       T2 *kernel, int kernelSize, bool allocate = true)
{
    ...
    //TBB parallel_for
    parallel_for(blocked_range<int>(0, h, GRAIN_SIZE), 
                            DoFlexFilterRowVTBB<T,T2>(&src, &dst, kernel, n));
...
template<class T, class T2>
bool FlexFilterColumnVTBB(FlexImage<T,1> &src, FlexImage<T,1> &dst, 
                          T2 *kernel, int kernelSize, bool allocate = true)
{
    if(kernelSize % 2 == 0) //enforce odd length kernels
        return false;
    ...
    //TBB parallel_for
    parallel_for(blocked_range<int>(n, h, GRAIN_SIZE), 
                    DoFlexFilterColumnVTBB<T,T2>(&src, &dst, kernel, n));
...
inline FlexImage8u GradientMagThresholdTBB(FlexImage8u &src, float threshold)
{
    ...
    //TBB parallel_for
    parallel_for(blocked_range<int>(1, src.Height() - 1), 
                 DoGradientMagThresholdTBB<Im8u>(&src, &r, threshold), auto_partitioner());
    return r; 
}
...
void TrackingModelTBB::CreateEdgeMap(FlexImage8u &src, FlexImage8u &dst)
{
    ...
    FlexImage8u gr = GradientMagThresholdTBB(src, 16.0f); 
    GaussianBlurTBB(&gr, &dst);	//Blur to create distance error map
}

//Generate an edge map from the original camera image
//This is exactly the same as the CreateEdgeMap except that it is not 
//a member function of TrackingModelTBB class
...
class DoProcessImages	{
    ...
    DoProcessImages(vector<string> *FGfiles, vector<string> *ImageFiles, 
                    ImageSet *edgeMaps, BinaryImageSet *FGMaps)
...
bool TrackingModelTBB::GetObservation(float timeval)
{
    ...
    for(int i = 0; i < n; i++)
    {	
        FGfiles[i] = mPath + "FG" + str(i + 1) + 
                     DIR_SEPARATOR + "image" + str(frame, 4) + ".bmp";
        ImageFiles[i] = mPath + "CAM" + str(i + 1) + 
                        DIR_SEPARATOR + "image" + str(frame, 4) + ".bmp";
    }
    FlexImage8u im;
...
void *TrackingModelTBB::operator()(void *inToken)
{
    ...
    {	
        FGfiles[i] = mPath + "FG" + str(i + 1) + DIR_SEPARATOR + 
                     "image" + str(mCurFrame, 4) + ".bmp";
        ImageFiles[i] = mPath + "CAM" + str(i + 1) + DIR_SEPARATOR + 
                     "image" + str(mCurFrame, 4) + ".bmp";
    }

    //TBB parallel_for
    parallel_for(blocked_range<int>(0, n), DoProcessImages(&FGfiles, &ImageFiles, 
                    &(token->edgeMaps), &(token->FGmaps)), auto_partitioner());
