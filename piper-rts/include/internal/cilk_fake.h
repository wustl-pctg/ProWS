 ...
 * The extended frame type should inherit from __cilk_fake_stack_frame.
 *
 * We are not doing shrink-wrapping for the pipe_while loop.
 */
#define CILK_FAKE_EXTENDED_FRAME_PROLOG(FrameType, fname)            \
    FrameType fname;                                                 \
    CILK_FAKE_FORCE_FRAME_PTR(fname);                                \
    __cilk_fake_enter_frame(&(fname))
